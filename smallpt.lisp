...
(define-compiler-macro +v (a b)
  (if (and (consp a)
           (member (car a) '(+v -v *v *s %v )))
      `(let ((.a ,a)
             (.b ,b))
         (setf (vec-x .a) (+ (vec-x .a) (vec-x .b))
               (vec-y .a) (+ (vec-y .a) (vec-y .b))
               (vec-z .a) (+ (vec-z .a) (vec-z .b)))
         .a)
      `(let ((.a ,a)
             (.b ,b))
         (vec (+ (vec-x .a) (vec-x .b))
...
(define-compiler-macro *s (a b)
  (if (and (consp a)
           (member (car a) '(+v -v *v *s %v )))
      `(let ((.a ,a)
             (.b ,b))
         (setf (vec-x .a) (* (vec-x .a) .b)
               (vec-y .a) (* (vec-y .a) .b)
               (vec-z .a) (* (vec-z .a) .b))
         .a)
      `(let ((.a ,a)
...
(define-compiler-macro %v (a b)
  (if (and (consp a)
      ...
      `(let ((.a ,a)
             (.b ,b))
         (setf (vec-x .a) (- (* (vec-y .a) (vec-z .b))
                             (* (vec-z .a) (vec-y .b)))
               (vec-y .a) (- (* (vec-z .a) (vec-x .b))
                             (* (vec-x .a) (vec-z .b)))
               (vec-z .a) (- (* (vec-x .a) (vec-y .b))
                             (* (vec-y .a) (vec-x .b))))
         .a)
      `(let ((.a ,a)
             (.b ,b))
...
(defun mollify (l rd n nl dist type molif_r)
  (declare (optimize (speed 3) (space 0) (debug 0))
           ...
           (type keyword type)
           (type vec l rd n nl))
  (let* ((cos_max (/ (sqrt (+ 1.0d0 (expt (/ molif_r dist) 2)))))
             ...
             (let ((f (*s f (/ p))))
               ...
               (loop for s in *lights* do
                 (let* ((sw (-v (sphere-pos s) x))
                        (su (normv (%v (if (> (abs (vec-x sw)) 0.1d0)
               ...
               (case (sphere-refl obj)
                 (:diff
                  (let* ((r1 (random (* 2.0d0 pi)))
                         ...
                         (r2s (sqrt r2))
                         (w nl)
                         (u (normv (%v (if (> (abs (vec-x w)) 0.1d0)
                                           (vec0 :y 1.0d0)
                                           (vec0 :x 1.0d0))
                                       w)))
                         (v (%v w u))
